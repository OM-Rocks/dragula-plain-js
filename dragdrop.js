var dropmodule = (function () {
			...
			function dragOver(zone, index, e) {
				return function() { zone.style.border = "1px dashed gray"; }.bind(this)();
			}
			
			function onFileInput(e) {
				var ele = e.target.closest('.dropzone');
				removeSiblings(ele);
				allfiles[ele.getAttribute('name') || ele.getAttribute('id')] = e.srcElement.files;
			...
			function addAdjacentFiles(ele, files, name) {
				...
				ele.innerHTML += `<div style="font-weight: 500; margin-top:80px;margin-left: -30px" class=${name}> Files attached is / are :</div><div style=" text-align: left !important" class=${name}>${fileStr}</div>`;
			}
